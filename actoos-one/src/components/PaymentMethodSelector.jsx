...
export function PaymentMethodSelector({ 
  ...
  const insufficientBalance = walletBalance < orderTotal;
  ...
  return (
    <div className="space-y-3" data-testid="payment-methods">
      {availableMethods.map((method) => {
        ...
        const isDisabled = method.isWallet && insufficientBalance;
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className={`font-semibold ${
                isSelected ? 'text-[#FF5A00]' : 'text-gray-900'
              }`}>
                {method.name}
              </p>
              ...
              {method.isWallet && insufficientBalance && (
                <p className="text-xs text-red-500 mt-1">
                  Solde insuffisant (manque {(orderTotal - walletBalance).toLocaleString()} FCFA)
                </p>
              )}
            </div>
