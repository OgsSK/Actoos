...
-- 7. POLICIES FAVORITES
CREATE POLICY fav_select ON favorites FOR SELECT USING (true);
CREATE POLICY fav_insert ON favorites FOR INSERT WITH CHECK (true);
CREATE POLICY fav_update ON favorites FOR UPDATE USING (true);
CREATE POLICY fav_delete ON favorites FOR DELETE USING (true);

-- 8. POLICIES RATINGS
CREATE POLICY rat_select ON ratings FOR SELECT USING (true);
CREATE POLICY rat_insert ON ratings FOR INSERT WITH CHECK (true);

-- 9. POLICIES PROMO_CODES
CREATE POLICY promo_select ON promo_codes FOR SELECT USING (true);
CREATE POLICY promo_update ON promo_codes FOR UPDATE USING (true);

-- 10. POLICIES PROMO_USAGE
CREATE POLICY usage_select ON promo_usage FOR SELECT USING (true);
CREATE POLICY usage_insert ON promo_usage FOR INSERT WITH CHECK (true);

-- 11. POLICIES DRIVER_LOCATIONS
CREATE POLICY loc_select ON driver_locations FOR SELECT USING (true);
CREATE POLICY loc_insert ON driver_locations FOR INSERT WITH CHECK (true);
CREATE POLICY loc_update ON driver_locations FOR UPDATE USING (true);
...
('BIENVENUE', '10 pourcent de reduction', 'percentage', 10, 2000, '2025-12-31 23:59:59', true),
('LIVRAISON', 'Livraison gratuite', 'free_delivery', 0, 3000, '2025-12-31 23:59:59', true),
('ACTOOS500', '500 FCFA de reduction', 'fixed', 500, 2500, '2025-12-31 23:59:59', true)
