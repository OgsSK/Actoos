...
async def create_checkout_session(request: Request, checkout_request: CheckoutRequest):
    ...
    metadata = {"package_id": package_id, "package_name": package["name"], "source": "actoos_jobs"}
    if checkout_request.job_id: metadata["job_id"] = checkout_request.job_id
    if checkout_request.user_email: metadata["user_email"] = checkout_request.user_email
    if checkout_request.user_id: metadata["user_id"] = checkout_request.user_id
    if checkout_request.metadata: metadata.update(checkout_request.metadata)
    try:
        if package["type"] == "subscription":
            mode = "subscription"
            line_item = {'price_data': {'currency': 'xof', 'product_data': {'name': package["name"]}, 'unit_amount': int(package["amount"]), 'recurring': {'interval': package["interval"]}}, 'quantity': 1}
        else:
            mode = "payment"
            line_item = {'price_data': {'currency': 'xof', 'product_data': {'name': package["name"]}, 'unit_amount': int(package["amount"])}, 'quantity': 1}
        session = stripe.checkout.Session.create(payment_method_types=['card'], line_items=[line_item], mode=mode, success_url=success_url, cancel_url=cancel_url, metadata=metadata)
        payment_transactions[session.id] = {"session_id": session.id, "package_id": package_id, "amount": package["amount"], "currency": "xof", "status": "pending", "payment_status": "initiated", "metadata": metadata}
    ...
    try:
        ...
        if session_id in payment_transactions:
            ...
            if tx["payment_status"] != "paid" and session.payment_status == "paid":
                tx["payment_status"] = "paid"; tx["status"] = "completed"
        return {"status": session.status, "payment_status": session.payment_status, "amount_total": session.amount_total, "currency": session.currency, "metadata": session.metadata}
    except stripe.error.StripeError as e:
        raise HTTPException(status_code=400, detail=str(e))
    ...
    try:
        ...
        if event.type == "checkout.session.completed":
            ...
            if supabase_url and supabase_key and session.metadata:
                user_id = session.metadata.get("user_id"); package_id = session.metadata.get("package_id")
                if user_id and package_id:
                    ...
                    if companies and len(companies) > 0:
                        company = companies[0]; plan_name = "free"
                        if "pro" in package_id: plan_name = "pro"
                        elif "business" in package_id: plan_name = "business"
                        update_data = {"subscription_plan": plan_name, "stripe_subscription_id": session.subscription, "stripe_customer_id": session.customer, "subscription_expires_at": None}
                        httpx.patch(f"{supabase_url}/rest/v1/companies?id=eq.{company['id']}", json=update_data, headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        elif event.type == "checkout.session.expired":
            session = event.data.object
            if session.id in payment_transactions: payment_transactions[session.id]["status"] = "expired"
        return {"received": True}
    except (ValueError, stripe.error.SignatureVerificationError) as e: raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        print(f"Webhook error: {e}"); return {"received": True, "error": str(e)}
...
async def contact_form(contact: ContactRequest):
    ...
    try:
        resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": ["contact@actoos.com"], "reply_to": contact.email, "subject": f"[Contact] {contact.subject}", "html": f"<h2>Nouveau message de {contact.name}</h2><p><strong>Email :</strong> {contact.email}</p><p><strong>Sujet :</strong> {contact.subject}</p><p><strong>Message :</strong></p><p>{contact.message}</p>"})
...
async def newsletter_unsubscribe(email: str = Query(...)):
    ...
    if not supabase_url or not supabase_key: raise HTTPException(status_code=500, detail="Supabase not configured")
    try:
        resp = httpx.get(f"{supabase_url}/rest/v1/newsletter_subscribers?email=eq.{email}&is_active=eq.true", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        if resp.status_code != 200 or not resp.json(): return {"success": False, "message": "Adresse non trouvée ou déjà désabonnée."}
        httpx.patch(f"{supabase_url}/rest/v1/newsletter_subscribers?email=eq.{email}", json={"is_active": False, "unsubscribed_at": "now()"}, headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
...
async def admin_send_newsletter(req: AdminNewsletterRequest):
    ...
    if not supabase_url or not supabase_key: raise HTTPException(status_code=500, detail="Supabase not configured")
    try:
        subscribers_resp = httpx.get(f"{supabase_url}/rest/v1/newsletter_subscribers?select=email&is_active=eq.true", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        subscribers = subscribers_resp.json()
        if not isinstance(subscribers, list) or len(subscribers) == 0: return {"success": True, "message": "Aucun abonné trouvé."}
        success_count = 0
        for sub in subscribers:
            ...
            try:
                resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [email], "subject": req.subject, "html": html_personalized})
                success_count += 1
            except Exception as e: print(f"Erreur envoi à {email}: {e}")
...
async def send_interview_link(req: SendInterviewLinkRequest):
    if not resend.api_key: raise HTTPException(status_code=500, detail="Email service not configured")
    try:
        safe_job_title = clean_subject(req.job_title); company_info = f" chez {req.company_name}" if req.company_name else ""
        resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [req.email], "subject": f"Entretien pour le poste : {safe_job_title}", "html": f"<h2>Bonjour {req.candidate_name},</h2><p>Vous êtes invité à un entretien pour le poste <strong>{req.job_title}</strong>{company_info}.</p><p>Voici le lien de visioconférence :</p><p><a href=\"{req.meeting_link}\">{req.meeting_link}</a></p><p>À bientôt,</p><p>L'équipe Actoos Jobs</p>"})
...
FALLBACK_MODELS = ["mistralai/mistral-7b-instruct:free", "google/gemini-2.0-flash-lite-preview-02-05:free", "google/gemma-2-9b-it:free", "meta-llama/llama-2-13b-chat:free", "microsoft/phi-3-mini-128k-instruct:free", "openai/gpt-3.5-turbo"]

@app.post("/api/ai/agent")
async def ai_agent(req: AIAgentRequest):
    if not OPENROUTER_API_KEY: raise HTTPException(status_code=500, detail="OpenRouter API key not configured")
    system_prompt = AGENT_PROMPTS.get(req.agent_id)
    if not system_prompt: raise HTTPException(status_code=400, detail="Invalid agent_id")
    messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": f"Texte à améliorer :\n\n{req.text}" + (f"\n\nContexte supplémentaire : {req.context}" if req.context else "")}]
    last_error = None
    for model in FALLBACK_MODELS:
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post("https://openrouter.ai/api/v1/chat/completions", headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}", "HTTP-Referer": "https://jobs.actoos.com", "X-Title": "Actoos Jobs AI"}, json={"model": model, "messages": messages, "temperature": 0.7, "max_tokens": 800})
                data = response.json()
                if "choices" in data and len(data["choices"]) > 0:
                    improved_text = data["choices"][0]["message"]["content"].strip()
                    return {"success": True, "result": improved_text, "model_used": model}
                if data.get("error", {}).get("code") == 429: continue
                last_error = data
        except Exception as e: print(f"Erreur avec {model}: {str(e)}"); continue
...
async def upload_file(req: UploadRequest):
    supabase_url = os.getenv("SUPABASE_URL"); supabase_key = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
    if not supabase_url or not supabase_key: raise HTTPException(status_code=500, detail="Supabase storage not configured")
    try:
...
async def upload_document(req: UploadDocumentRequest):
    ...
    try:
        ...
        if insert_response.status_code not in [200, 201]: raise Exception(f"Insert failed: {insert_response.text}")
...
async def notify_new_application(req: NotifyNewApplicationRequest):
    if not resend.api_key: raise HTTPException(status_code=500, detail="Email service not configured")
    try:
        resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [req.recruiter_email], "subject": f"Nouvelle candidature pour {clean_subject(req.job_title)}", "html": f"<h2>Bonjour {req.recruiter_name},</h2><p><strong>{req.candidate_name}</strong> vient de postuler à votre offre <strong>{req.job_title}</strong>.</p><p>Consultez la candidature sur <a href=\"https://jobs.actoos.com/dashboard/entreprise/candidatures\">Actoos Jobs</a>.</p>"})
...
async def send_job_alerts():
    ...
    if not isinstance(alerts, list) or len(alerts) == 0: return {"success": True, "message": "Aucune alerte active"}
    count_sent = 0
    for alert in alerts:
        user_email = alert.get("user", {}).get("email"); user_id = alert.get("user_id")
        if not user_email or not user_id: continue
        ...
        jobs = rpc_resp.json()
        if not isinstance(jobs, list) or len(jobs) == 0: continue
        ...
        try:
            ...
            count_sent += 1
        except Exception as e: print(f"Erreur envoi alerte {alert.get('id')}: {e}")
...
async def admin_verify_company(req: AdminVerifyCompanyRequest):
    ...
    try:
        ...
        if not isinstance(companies, list) or len(companies) == 0: raise HTTPException(status_code=404, detail="Entreprise non trouvée")
        company = companies[0]
        httpx.patch(f"{supabase_url}/rest/v1/companies?id=eq.{req.id}", json={"is_verified": True}, headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        owner = company.get("owner", {}); owner_email = owner.get("email"); owner_first_name = owner.get("first_name") or "Cher recruteur"
        if owner_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [owner_email], "subject": "Votre entreprise a été validée", "html": f"<h2>Félicitations {owner_first_name} !</h2><p>Votre entreprise <strong>{company['name']}</strong> a été validée par notre équipe. Vous pouvez maintenant publier des offres et recevoir des candidatures.</p><p><a href=\"https://jobs.actoos.com/dashboard/entreprise\">Accéder à mon espace recruteur</a></p>"})
...
async def notify_admin_new_company(req: NewCompanyNotificationRequest):
    if not resend.api_key: raise HTTPException(status_code=500, detail="Email service not configured")
    try:
        resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": ["contact@actoos.com"], "subject": f"Nouvelle entreprise à valider : {req.company_name}", "html": f"<h2>Nouvelle entreprise en attente de validation</h2><p><strong>Entreprise :</strong> {req.company_name}</p><p><strong>Propriétaire :</strong> {req.owner_name} ({req.owner_email})</p><p><a href=\"https://jobs.actoos.com/admin\">Accéder au dashboard admin</a></p>"})
...
async def notify_admin_new_job(req: NotifyAdminNewJobRequest):
    if not resend.api_key: raise HTTPException(status_code=500, detail="Email service not configured")
    try:
        resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": ["contact@actoos.com"], "subject": f"Nouvelle offre à valider : {req.job_title}", "html": f"<h2>Nouvelle offre en attente de validation</h2><p><strong>Offre :</strong> {req.job_title}</p><p><strong>Entreprise :</strong> {req.company_name} ({req.company_email})</p><p><a href=\"https://jobs.actoos.com/admin\">Accéder au dashboard admin</a></p>"})
        return {"success": True, "message": "Notification envoyée à l'administrateur"}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
async def admin_delete_company(company_id: str):
    ...
    if not supabase_url or not supabase_key: raise HTTPException(status_code=500, detail="Supabase not configured")
    try:
        company_resp = httpx.get(f"{supabase_url}/rest/v1/companies?id=eq.{company_id}&select=id,name,owner:users(email,first_name)", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        companies = company_resp.json()
        if not companies: raise HTTPException(status_code=404, detail="Entreprise non trouvée")
        company = companies[0]; owner_email = company.get("owner", {}).get("email"); owner_first_name = company.get("owner", {}).get("first_name") or "Utilisateur"
        httpx.delete(f"{supabase_url}/rest/v1/companies?id=eq.{company_id}", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        if owner_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [owner_email], "subject": "Votre entreprise a été supprimée", "html": f"<h2>Bonjour {owner_first_name},</h2><p>Votre entreprise <strong>{company['name']}</strong> a été supprimée par l'administrateur.</p><p>Si vous pensez qu'il s'agit d'une erreur, veuillez contacter notre support.</p>"})
...
async def admin_delete_user(user_id: str):
    ...
    if not supabase_url or not supabase_key: raise HTTPException(status_code=500, detail="Supabase not configured")
    try:
        user_resp = httpx.get(f"{supabase_url}/rest/v1/users?id=eq.{user_id}&select=email,first_name", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        users = user_resp.json()
        if not users: raise HTTPException(status_code=404, detail="Utilisateur non trouvé")
        user_email = users[0].get("email"); user_first_name = users[0].get("first_name") or "Utilisateur"
        httpx.delete(f"{supabase_url}/rest/v1/users?id=eq.{user_id}", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        httpx.delete(f"{supabase_url}/auth/v1/admin/users/{user_id}", headers={"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"})
        if user_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [user_email], "subject": "Votre compte a été supprimé", "html": f"<h2>Bonjour {user_first_name},</h2><p>Votre compte sur Actoos Jobs a été supprimé par l'administrateur.</p><p>Si vous pensez qu'il s'agit d'une erreur, veuillez contacter notre support.</p>"})
...
async def admin_reject_company(req: AdminActionRequest):
    ...
    try:
        ...
        if owner_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [owner_email], "subject": "Votre entreprise a été refusée", "html": f"<h2>Désolé {company['owner']['first_name']}</h2><p>Votre entreprise <strong>{company['name']}</strong> n'a pas été validée.</p><p><strong>Raison :</strong> {req.reason or 'Non spécifiée'}</p>"})
...
async def admin_suspend_company(req: AdminActionRequest):
    ...
    try:
        ...
        owner_email = company.get("owner", {}).get("email")
        if owner_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [owner_email], "subject": "Votre entreprise a été suspendue", "html": f"<h2>Information importante</h2><p>Votre entreprise <strong>{company['name']}</strong> a été suspendue.</p><p><strong>Raison :</strong> {req.reason or 'Non spécifiée'}</p>"})
...
async def admin_suspend_job(req: AdminActionRequest):
    ...
    try:
        ...
        jobs = job_resp.json()
        if not isinstance(jobs, list) or len(jobs) == 0: raise HTTPException(status_code=404, detail="Offre non trouvée")
        job = jobs[0]
        ...
        if owner_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [owner_email], "subject": "Votre offre a été suspendue", "html": f"<h2>Votre offre \"{job['title']}\" a été suspendue</h2><p><strong>Raison :</strong> {req.reason or 'Non spécifiée'}</p>"})
...
async def admin_delete_job(req: AdminActionRequest):
    ...
    try:
        ...
        owner_email = job.get("posted_by_user", {}).get("email")
        if owner_email and resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [owner_email], "subject": "Votre offre a été supprimée", "html": f"<h2>Votre offre \"{job['title']}\" a été supprimée par l'administrateur</h2><p><strong>Raison :</strong> {req.reason or 'Non spécifiée'}</p>"})
...
async def update_report_status(report_id: str, status: str = "reviewed"):
    ...
    try:
        ...
        if response.status_code != 200: raise Exception(f"Report update failed: {response.text}")
...
async def toggle_user_status(req: AdminToggleUserStatusRequest):
    ...
    try:
        ...
        users = user_resp.json()
        if not isinstance(users, list) or len(users) == 0: raise HTTPException(status_code=404, detail="Utilisateur non trouvé")
        user = users[0]
...
async def ban_user(req: AdminBanUserRequest):
    ...
    try:
        ...
        if resend.api_key: resend.Emails.send({"from": "Actoos Jobs <noreply@actoos.com>", "to": [user["email"]], "subject": "Votre compte a été banni", "html": f"<h2>Bonjour {user['first_name']},</h2><p>Votre compte sur Actoos Jobs a été banni définitivement.</p><p><strong>Raison :</strong> {req.reason or 'Non spécifiée'}</p>"})
...
async def get_blog_posts(audience: Optional[str] = None):
    ...
    if audience and audience != "all": posts = [p for p in posts if p.get("audience") == audience or p.get("audience") == "all"]
    return posts
...
async def get_blog_post(slug: str):
    posts = load_blog_posts()
    for post in posts:
        if post.get("slug") == slug: return post
...
async def generate_blog_post(req: BlogGenerateRequest):
    ...
    if req.keywords: prompt += f"Mots-clés : {req.keywords}\n"
    prompt += f"Audience : {req.audience}\n"
    messages = [{"role": "system", "content": AGENT_PROMPTS["blog-post"]}, {"role": "user", "content": prompt}]
    async with httpx.AsyncClient(timeout=60.0) as client:
        for model in FALLBACK_MODELS:
            try:
                response = await client.post("https://openrouter.ai/api/v1/chat/completions", headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}", "HTTP-Referer": "https://jobs.actoos.com", "X-Title": "Actoos Jobs AI"}, json={"model": model, "messages": messages, "temperature": 0.8, "max_tokens": 1500})
                data = response.json()
                if "choices" in data and len(data["choices"]) > 0:
                    text = data["choices"][0]["message"]["content"].strip()
                    try: article = json.loads(text)
                    except: article = {"title": req.title, "excerpt": req.keywords or "Article généré automatiquement", "content": f"<p>{text}</p>", "category": req.category}
                    slug = req.title.lower().replace(" ", "-")[:80]
                    posts = load_blog_posts()
                    new_id = max([p.get("id", 0) for p in posts], default=0) + 1
                    new_post = {"id": new_id, "title": article.get("title", req.title), "slug": slug, "excerpt": article.get("excerpt", ""), "content": article.get("content", ""), "category": article.get("category", req.category), "audience": req.audience, "read_time": req.read_time, "author": req.author, "icon": req.icon, "color": req.color, "published_at": datetime.utcnow().isoformat()}
                    posts.append(new_post)
                    save_blog_posts(posts)
                    return new_post
            except Exception as e: print(f"Erreur modèle {model}: {e}"); continue
...
async def delete_blog_post(slug: str):
    ...
    if len(posts) == initial_len: raise HTTPException(status_code=404, detail="Article non trouvé")
    save_blog_posts(posts)
    return {"success": True}
...
def compute_match_score(job: dict, candidate_profile: dict) -> int:
    ...
    job_lvl = job.get("experience_level"); cand_lvl = candidate_profile.get("experience_level")
    ...
    if j_min and j_max and c_min and c_max:
        if c_min <= j_max and c_max >= j_min:
            ...
            if range_job > 0: score += (overlap / range_job) * 15
            else: score += 15
        elif c_min <= j_max or c_max >= j_min: score += 5
    job_city = job.get("city_id"); cand_city = candidate_profile.get("city_id")
    is_remote = job.get("is_remote", False); cand_remote = candidate_profile.get("is_open_to_remote", False)
    if job_city and cand_city and str(job_city) == str(cand_city): score += 15
    elif is_remote and cand_remote: score += 10
    else:
        job_country = job.get("country_id"); cand_country = candidate_profile.get("country_id")
        if job_country and cand_country and str(job_country) == str(cand_country): score += 5
    job_contract = job.get("contract_type"); cand_contracts = candidate_profile.get("preferred_contract_types") or []
    if isinstance(cand_contracts, list) and job_contract in cand_contracts: score += 10
    ...
    if isinstance(education, list) and education and job_req:
        ...
        if matches: score += min(10, matches * 3)
...
async def get_match_score(job_id: str, user_id: str = Query(...)):
    ...
    jobs = job_resp.json()
    if not jobs: raise HTTPException(status_code=404, detail="Offre non trouvée")
    job = jobs[0]
...
async def delete_own_account(request: Request):
    auth_header = request.headers.get("authorization")
    if not auth_header: raise HTTPException(status_code=401, detail="Non authentifié")
    supabase_url = os.getenv("SUPABASE_URL"); supabase_key = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
    headers = {"apikey": supabase_key, "Authorization": f"Bearer {supabase_key}"}
    token = auth_header.replace("Bearer ", "")
    user_resp = httpx.get(f"{supabase_url}/auth/v1/user", headers={"Authorization": f"Bearer {token}"})
    if user_resp.status_code != 200: raise HTTPException(status_code=401, detail="Token invalide")
    user_data = user_resp.json(); user_id = user_data.get("id")
    if not user_id: raise HTTPException(status_code=401, detail="Utilisateur introuvable")
    try:
        ...
        for company in companies: httpx.delete(f"{supabase_url}/rest/v1/companies?id=eq.{company['id']}", headers=headers)
