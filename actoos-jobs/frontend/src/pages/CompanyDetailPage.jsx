  ...
  <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 animate-pulse">
    <div className="bg-gradient-to-br from-slate-700 via-blue-800 to-slate-800 p-6 sm:p-10">
      <div className="flex flex-col sm:flex-row items-start gap-5 sm:gap-6">
        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-white/10 shrink-0" />
...
const CompanyDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      try {
        ...
        setCompany(compData);
        setFollowersCount(compData.followers_count || 0);

        if (!compData.is_active || (compData.owner && compData.owner.is_banned)) {
          setSuspended(true);
          return;
        }
  ...
  useEffect(() => {
    if (!TABS.find(t => t.key === activeTab)) setActiveTab('about');
  }, [TABS, activeTab]);

  if (loading) return <CompanySkeleton />;

  if (!company) return <div className="pt-20 text-center">{t('companyDetail.notFoundMessage')}</div>;
      ...
      <div className="min-h-screen bg-slate-50 pt-20">
        <div className="max-w-4xl mx-auto px-4 py-8 text-center">
          ...
          <button onClick={handleBack} className="mt-6 inline-flex items-center text-sm text-slate-600 hover:text-slate-900">
            <ChevronLeft className="w-4 h-4 mr-1" />{t('companyDetail.backToCompanies')}
          </button>
        </div>
      </div>
    );
  ...
  const telLink = phoneCode && company.phone
    ...
    <div className="min-h-screen bg-slate-50 pt-16 sm:pt-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start gap-5 sm:gap-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-6">
                  ...
                  <ShareButton url={window.location.origin + `/entreprises/${company.id}`} title={company.name} text={t('companyDetail.shareText', { name: company.name })} />
                  {!isOwner && user && !isAdmin && <ReportButton itemType="company" itemId={company.id} reporterId={user.id} className="bg-white/20 text-white hover:bg-white/30 rounded-full px-3 py-1 text-sm" />}
                  {isOwner && <Badge variant="outline" className="text-white/60 border-white/20">{t('companyDetail.yourCompany')}</Badge>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 sm:p-8">
              ...
              <div className="space-y-10">
                {company.description && (
                  <div className="bg-slate-50 rounded-2xl p-6 sm:p-8 border border-slate-100">
                    <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">{t('companyDetail.about')}</h3>
                    <p className="text-slate-700 leading-relaxed whitespace-pre-line text-base">{company.description}</p>
                  </div>
                )}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {company.city && <InfoCard icon={<MapPin className="w-5 h-5" />} label={t('companyDetail.city', 'Ville')} value={company.city.name} />}
                  {company.country && <InfoCard icon={<MapPinned className="w-5 h-5" />} label={t('companyDetail.country', 'Pays')} value={t(`countries.${company.country.code}`, company.country.name)} />}
                  {company.size && <InfoCard icon={<Users className="w-5 h-5" />} label={t('companyDetail.employees', { size: company.size })} value={company.size} />}
                  {company.founded_year && <InfoCard icon={<Calendar className="w-5 h-5" />} label={t('companyProfile.labels.foundedYear')} value={company.founded_year} />}
                </div>
              </div>
            )}
                ...
                {jobs.length === 0 ? (
                  ...
                  <div className="grid gap-4">
                    {jobs.map(job => {
                      ...
                      return (
                        <Link key={job.id} to={`/emplois/${job.id}`} className="block bg-white rounded-2xl p-5 border border-slate-100 hover:shadow-md hover:border-blue-200 transition-all">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-2 text-sm text-slate-500">
                                ...
                                {job.salary_min && job.salary_max && <span className="flex items-center gap-1"><Banknote className="w-4 h-4" />{format(job.salary_min)} – {format(job.salary_max)}</span>}
            ...
            {activeTab === 'news' && companyPosts.length > 0 && (
              <div className="animate-fadeIn">
                <h2 className="text-xl font-bold text-slate-900 mb-6">{t('candidateProfile.posts', 'Actualités')} ({companyPosts.length})</h2>
        ...
        {similarLoading ? (
          ...
          <div className="mt-16">
            <h2 className="text-2xl font-bold text-slate-900 mb-6">{t('companyDetail.similarCompanies', 'Entreprises similaires')}</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {similarCompanies.map(comp => (
                <Link key={comp.id} to={`/entreprises/${comp.id}`} className="block bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center overflow-hidden shrink-0">
                      {comp.logo_url ? <img src={comp.logo_url} alt={comp.name} className="w-full h-full object-cover" /> : <Building2 className="w-6 h-6 text-slate-400" />}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-semibold text-slate-900 truncate">{comp.name}</h3>
                      {comp.industry && <p className="text-xs text-slate-500">{getTranslatedIndustry(comp.industry)}</p>}
                      <p className="text-xs text-slate-400 mt-1">{t('companyDetail.jobsCount', { count: formatCount(comp.jobs_count || 0) })}</p>
                    </div>
                  </div>
                </Link>
              ))}
