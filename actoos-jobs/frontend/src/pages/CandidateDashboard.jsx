...
const SavedJobCard = ({ job, onRemove }) => {
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors">
      ...
      <Badge className={cn(contractInfo.color, 'border-0')}>{contractInfo.label}</Badge>
      <Button variant="ghost" size="sm" onClick={() => onRemove(job.id)} className="text-slate-400 hover:text-red-500"><Heart className="w-4 h-4 fill-current" /></Button>
    </div>
  );
};
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative w-20 h-20">
          <svg className="w-20 h-20 transform -rotate-90">
            ...
            <circle cx="40" cy="40" r="35" stroke={completion >= 80 ? '#22c55e' : completion >= 50 ? '#f59e0b' : '#3b82f6'} strokeWidth="6" fill="none" strokeDasharray={`${completion * 2.2} 220`} strokeLinecap="round" />
          </svg>
        ...
        <div className="flex-1">
          ...
          <Link to="/profil"><Button size="sm" variant="outline">Compléter mon profil<ChevronRight className="w-4 h-4 ml-1" /></Button></Link>
        </div>
      </div>
    </CardContent>
  </Card>
...
const CandidateDashboard = () => {
  ...
  const getTip = (completion) => {
    const lowProfileTips = [
      "Ajoutez une photo de profil : les profils avec photo reçoivent 40 % de contacts en plus.",
    ...
    <div className="min-h-screen bg-slate-50 pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard icon={Target} label="Taux de complétude" value={`${profileCompletion}%`} color="green" />
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-slate-200">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div><CardTitle className="text-lg">Mes candidatures</CardTitle><CardDescription>Suivez l'état de vos candidatures</CardDescription></div>
                <Link to="/mes-candidatures"><Button variant="ghost" size="sm">Voir tout<ChevronRight className="w-4 h-4 ml-1" /></Button></Link>
              </CardHeader>
              <CardContent>
                {applications.length === 0 ? (
                  <div className="text-center py-8"><FileText className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-slate-600 mb-4">Aucune candidature pour le moment</p><Link to="/emplois"><Button>Trouver un emploi</Button></Link></div>
                ) : (
            ...
            <Card className="border-slate-200">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div><CardTitle className="text-lg">Offres sauvegardées</CardTitle><CardDescription>Les offres qui vous intéressent</CardDescription></div>
                <Link to="/offres-sauvegardees"><Button variant="ghost" size="sm">Voir tout<ChevronRight className="w-4 h-4 ml-1" /></Button></Link>
              </CardHeader>
              <CardContent>
                {savedJobs.length === 0 ? (
                  <div className="text-center py-8"><Heart className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-slate-600 mb-4">Aucune offre sauvegardée</p><Link to="/emplois"><Button variant="outline">Parcourir les offres</Button></Link></div>
                ) : (
          ...
          <div className="space-y-6">
            ...
            {/* Quick Actions */}
            <Card className="border-slate-200">
              <CardHeader className="pb-2"><CardTitle className="text-base">Actions rapides</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                <Link to="/profil" className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors">
                  ...
                  <div><p className="font-medium text-slate-900">Mettre à jour mon CV</p><p className="text-xs text-slate-500">Téléchargez votre dernier CV</p></div>
                </Link>
                <Link to="/alertes" className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors">
                  ...
                  <div><p className="font-medium text-slate-900">Créer une alerte</p><p className="text-xs text-slate-500">Recevez les offres par email</p></div>
                </Link>
                <Link to="/parametres" className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors">
                  ...
                  <div><p className="font-medium text-slate-900">Paramètres</p><p className="text-xs text-slate-500">Gérez votre compte</p></div>
                </Link>
              </CardContent>
            </Card>
            ...
            <Card className="border-blue-200 bg-blue-50">
              <CardContent className="p-5">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center shrink-0"><BookOpen className="w-5 h-5 text-blue-600" /></div>
                  <div>
