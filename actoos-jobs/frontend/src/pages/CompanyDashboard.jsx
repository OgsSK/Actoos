        ...
        <div>
          ...
          {trend !== undefined && (
            <p className={cn('text-xs mt-1 flex items-center gap-1', trend > 0 ? 'text-green-600' : 'text-slate-500')}>
              <TrendingUp className="w-3 h-3" />{trend > 0 ? '+' : ''}{trend}% ce mois
            </p>
          )}
        </div>
        <div className={cn('w-12 h-12 rounded-xl flex items-center justify-center', color === 'blue' && 'bg-blue-100', color === 'green' && 'bg-green-100', color === 'purple' && 'bg-purple-100', color === 'orange' && 'bg-orange-100')}>
          <Icon className={cn('w-6 h-6', color === 'blue' && 'text-blue-600', color === 'green' && 'text-green-600', color === 'purple' && 'text-purple-600', color === 'orange' && 'text-orange-600')} />
        </div>
      </div>
    </CardContent>
  </Card>
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-20">
                ...
                {job.status === 'active' ? (
                  ...
                  <button onClick={() => { onToggleStatus(job, 'active'); setShowMenu(false); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-green-600 hover:bg-slate-50"><CheckCircle className="w-4 h-4" />Republier</button>
                ) : null}
                <button onClick={() => { onDelete(job); setShowMenu(false); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-slate-50"><Trash2 className="w-4 h-4" />Supprimer</button>
              </div>
            </>
          )}
        </div>
...
const CompanyDashboard = () => {
  ...
  const fetchCompanyData = async () => {
    ...
    setLoading(true);
    try {
      const { data: ownedCompany } = await supabase.from('companies').select('*').eq('owner_id', user.id).maybeSingle();
      if (ownedCompany) {
        setCompany(ownedCompany);
        const { data: jobsData } = await supabase.from('jobs').select('*, city:cities(name)').eq('company_id', ownedCompany.id).order('created_at', { ascending: false }).limit(10);
        setJobs(jobsData || []);
        if (jobsData?.length) {
          const { data: appsData } = await supabase.from('applications').select('*, candidate:users(first_name, last_name, email), job:jobs(title)').in('job_id', jobsData.map(j => j.id)).order('created_at', { ascending: false }).limit(10);
          setApplications(appsData || []);
        } else setApplications([]);
        const activeJobs = (jobsData || []).filter(j => j.status === 'active').length;
        setStats({ totalJobs: (jobsData || []).length, activeJobs, totalApplications: (applications || []).length, newApplications: (applications || []).filter(a => a.status === 'pending').length });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className="border-slate-200">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <div><h2 className="text-lg font-semibold text-slate-900">Mes offres d'emploi</h2><p className="text-sm text-slate-500">{stats.totalJobs} offres au total</p></div>
                <Link to="/dashboard/entreprise/offres"><Button variant="ghost" size="sm">Voir tout<ChevronRight className="w-4 h-4 ml-1" /></Button></Link>
              </div>
              <CardContent className="p-6">
                {jobs.length === 0 ? (
                  <div className="text-center py-8"><Briefcase className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-slate-600 mb-4">Aucune offre publiée</p><Link to="/dashboard/entreprise/offres/nouvelle"><Button>Publier une offre</Button></Link></div>
                ) : (
                  <div className="space-y-3">{jobs.map(job => <CompanyJobCard key={job.id} job={job} onEdit={handleEditJob} onDelete={handleDeleteJob} onToggleStatus={handleToggleJobStatus} />)}</div>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="border-slate-200">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <div><h2 className="text-lg font-semibold text-slate-900">Candidatures récentes</h2><p className="text-sm text-slate-500">{stats.newApplications} nouvelles</p></div>
                <Link to="/dashboard/entreprise/candidatures"><Button variant="ghost" size="sm">Voir tout<ChevronRight className="w-4 h-4 ml-1" /></Button></Link>
              </div>
              <CardContent className="p-4">
                {applications.length === 0 ? (
                  <div className="text-center py-6"><Users className="w-10 h-10 text-slate-300 mx-auto mb-2" /><p className="text-sm text-slate-500">Aucune candidature</p></div>
                ) : (
                  <div className="space-y-2">{applications.slice(0, 5).map(app => <ApplicationCard key={app.id} application={app} />)}</div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border-slate-200">
              <CardContent className="p-6">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {company.website && <a href={company.website.startsWith('http') ? company.website : `https://${company.website}`} target="_blank" className="flex items-center gap-2 text-blue-600 hover:underline"><Globe className="w-4 h-4" />{company.website}</a>}
                  {company.email && <a href={`mailto:${company.email}`} className="flex items-center gap-2 text-blue-600 hover:underline"><Mail className="w-4 h-4" />{company.email}</a>}
                  {company.phone && <a href={`tel:${company.phone}`} className="flex items-center gap-2 text-slate-600 hover:text-blue-600"><Phone className="w-4 h-4 text-slate-400" />{company.phone}</a>}
                ...
                <Link to="/dashboard/entreprise/profil"><Button variant="outline" className="w-full mt-4">Modifier le profil</Button></Link>
              </CardContent>
            </Card>
