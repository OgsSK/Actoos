...
import { Loader2, ChevronLeft, Heart, MapPin } from 'lucide-react';
...
const SavedJobsPage = () => {
  ...
  const fetchSaved = async () => {
    ...
    const { data } = await supabase
      .from('saved_jobs')
      .select('job:jobs(id, title, contract_type, company:companies(name), city:cities(name))')
      .eq('user_id', user.id)
    ...
    <div className="min-h-screen bg-slate-50 pt-20">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Offres sauvegardées</h1>
        </div>
        {jobs.length === 0 ? (
          <Card><CardContent className="p-8 text-center text-slate-500"><Heart className="w-12 h-12 mx-auto mb-4" />Aucune offre sauvegardée.</CardContent></Card>
        ) : (
