...
const JobDetailPage = () => {
  ...
  const fetchMatchScore = async () => {
    try {
      const res = await apiFetch(
        `/api/jobs/${job.id}/match-score?user_id=${user.id}`
      );
  ...
  const handleApply = async () => {
    ...
    if (isCompany) {
      toast.error(
        'Les entreprises ne peuvent pas postuler'
      );
      return;
    }
  ...
  const handleToggleSave = async () => {
    ...
    if (isCompany) {
      toast.error(
        'Les entreprises ne peuvent pas ajouter des favoris'
      );
      return;
    }
    ...
    <Badge className="bg-blue-100 text-blue-700 text-sm px-3 py-1">
      🎯 {matchScore}% de correspondance
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  ...
                  <Badge className={contractInfo.color}>
                    {contractInfo.label}
                  </Badge>
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                ...
                {/* Actions candidats uniquement */}
{!isOwner && !isCompany && !isAdmin && (   // ← ajout de && !isAdmin
  <>
    {hasApplied ? (
