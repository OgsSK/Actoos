...
const JobCard = ({ job, onEdit, onDelete, onToggleStatus }) => {
  ...
  const contractInfo =
    CONTRACT_TYPES[job.contract_type] || CONTRACT_TYPES.cdi;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-sm text-slate-500">
            ...
            <Badge
              className={`${contractInfo.color} border-0 text-xs`}
            >
              {contractInfo.label}
            </Badge>
...
const CompanyJobsPage = () => {
  ...
  const handleDeleteJob = async (job) => {
    if (
      !window.confirm(
        `Êtes-vous sûr de vouloir supprimer "${job.title}" ?`
      )
    )
      return;
    ...
    <div className="min-h-screen bg-slate-50 pt-16 sm:pt-20">
      <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8">
          ...
          <Link
            to="/dashboard/entreprise/offres/nouvelle"
