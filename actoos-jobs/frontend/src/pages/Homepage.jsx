...
const CategoriesSection = () => {
  ...
  return (
    <section className="py-20 bg-white"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="text-center mb-12"><h2 className="text-3xl sm:text-4xl font-bold text-slate-900 font-display">Explorez par catégorie</h2><p className="text-slate-600 mt-3 max-w-2xl mx-auto">Trouvez des opportunités dans votre domaine d'expertise</p></div><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">{categories.map((category) => (<Link key={category.id} to={`/emplois?category=${category.slug}`} className="group bg-slate-50 hover:bg-blue-50 border border-slate-100 hover:border-blue-200 rounded-3xl p-5 text-center transition-all duration-300 hover:shadow-lg hover:-translate-y-1"><div className="text-4xl mb-3">{category.icon || '📌'}</div><h3 className="font-semibold text-slate-900 group-hover:text-blue-600 text-sm">{category.name}</h3></Link>))}</div></div></section>
  );
};
...
const RecentJobsSection = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const { data, error } = await supabase.from('jobs').select(`id, title, contract_type, salary_min, salary_max, created_at, is_urgent, company:companies(name, logo_url, owner_id), city:cities(name)`).eq('status', 'active').order('created_at', { ascending: false }).limit(6);
        if (error) throw error;
        const formattedJobs = (data || []).map((job) => ({ id: job.id, title: job.title, company: job.company?.name || 'Entreprise', company_logo: job.company?.logo_url, owner_id: job.company?.owner_id, location: job.city?.name || 'Non spécifié', contract_type: job.contract_type, salary_min: job.salary_min, salary_max: job.salary_max, created_at: job.created_at, urgent: job.is_urgent }));
  ...
  return (
    <Link to={`/emplois/${job.id}`} className="block group">
      <Card className="hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-slate-200 rounded-3xl overflow-hidden bg-white relative">
        {job.urgent && <div className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs font-medium px-3 py-1 text-center">🔥 Urgent - Postulez maintenant</div>}
        <CardContent className="p-5 pt-3">
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100"><div className="text-sm"><span className="text-slate-500">Salaire: </span><span className="font-medium text-slate-700">{job.salary_min && job.salary_max ? `${job.salary_min.toLocaleString('fr-FR')} - ${job.salary_max.toLocaleString('fr-FR')} FCFA` : 'Non précisé'}</span></div><span className="text-xs text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3" />{formatRelative(job.created_at)}</span></div>
        </CardContent>
        {isOwner && <div className="absolute top-2 left-2"><Badge className="bg-blue-600 text-white text-xs">Votre offre</Badge></div>}
        {!isOwner && !isCompany && (
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="text-center lg:text-left">
            <Badge className="bg-white/10 text-white border border-blue-400/30 mb-6 rounded-full px-4 py-2">Pour les entreprises</Badge>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white font-display mb-6 leading-tight">Recrutez les meilleurs talents</h2>
            <p className="text-blue-100 text-lg leading-relaxed mb-8 max-w-xl mx-auto lg:mx-0">Publiez vos offres, recevez des candidatures qualifiées et trouvez votre prochain collaborateur en quelques jours.</p>
            ...
            <div className="relative w-[420px] h-[420px]">
              <div className="absolute top-0 left-0 z-20 bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl px-5 py-4 border border-white/20">
                <div className="flex items-center gap-4">
                  ...
                  <div><p className="text-2xl font-bold text-slate-900">{activeJobs !== null ? `${activeJobs}+` : '...'}</p><p className="text-sm text-slate-500">Offres actives</p></div>
                </div>
              </div>
              <div className="absolute bottom-0 right-0 z-20 bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl px-5 py-4 border border-white/20">
                <div className="flex items-center gap-4">
                  ...
                  <div><p className="text-2xl font-bold text-slate-900">{candidates !== null ? `${candidates}+` : '...'}</p><p className="text-sm text-slate-500">Candidats</p></div>
                </div>
              </div>
              <div className="absolute top-1/2 left-0 -translate-y-1/2 z-20 bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl px-5 py-4 border border-white/20">
                <div className="flex items-center gap-4">
                  ...
                  <div><p className="text-2xl font-bold text-slate-900">{companies !== null ? `${companies}+` : '...'}</p><p className="text-sm text-slate-500">Entreprises</p></div>
                </div>
              </div>
...
const WhyChooseSection = () => {
  ...
  return (
    <section className="py-20 bg-slate-50"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="text-center mb-12"><h2 className="text-3xl sm:text-4xl font-bold text-slate-900 font-display">Pourquoi choisir Actoos Jobs ?</h2><p className="text-slate-600 mt-3 max-w-2xl mx-auto">La plateforme de recrutement nouvelle génération, créée par des ingénieurs passionnés.</p></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">{reasons.map((reason) => (<Card key={reason.title} className="border-0 shadow-lg bg-white rounded-3xl hover:shadow-xl transition-all duration-300"><CardContent className="p-6 text-center"><div className="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><reason.icon className="w-7 h-7 text-blue-600" /></div><h3 className="font-semibold text-slate-900 mb-2">{reason.title}</h3><p className="text-slate-600 text-sm">{reason.description}</p></CardContent></Card>))}</div><div className="text-center mt-12"><Link to="/inscription"><Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white rounded-2xl">Créer un compte gratuit <ArrowRight className="w-4 h-4 ml-2" /></Button></Link></div></div></section>
  );
};
