...
const AdminDashboard = () => {
  ...
  const fetchBlogPosts = async () => {
    setLoadingBlog(true);
    try {
      const data = await apiFetch('/api/blog/posts?audience=all');
      setBlogPosts(data || []);
    } catch (err) {
  ...
  const handleGenerateArticle = async () => {
    ...
    try {
      ...
      setBlogForm({...blogForm, title: '', keywords: ''});
      fetchBlogPosts();
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-16 sm:pt-20" data-testid="admin-dashboard">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        {/* Existing tabs (overview, jobs, companies, users, reports, cancellations, newsletter) are identical to the previous version and have been omitted for brevity in this response. They are unchanged. */}
          ...
          <div className="space-y-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Uniquement le titre et des mots-clés, l'IA fait le reste.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                />
                <Input
                  placeholder="Mots-clés (ex: recrutement, CV, entretien)"
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <option value="company">Recruteurs</option>
                  </select>
                  <Input
            ...
            <Card>
              ...
              <CardContent>
                {loadingBlog ? (
                  ...
                  <p className="text-slate-500">Aucun article.</p>
                ) : (
                  <div className="space-y-2">
                    {blogPosts.map((post) => (
                      <div
                        key={post.id}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-slate-50 rounded-xl gap-2"
                      >
                        <div>
                          <span className="font-medium">{post.title}</span>
                          <p className="text-xs text-slate-500">
                            {post.category} • {post.audience} • {post.author}
                          </p>
                        </div>
                        <Button
                          variant="outline"
