        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              <div key={i} className="bg-white rounded-2xl p-6 border border-slate-200 hover:border-slate-300 transition-colors">
                <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center mb-4">
                  <cap.icon size={18} className="text-slate-700" />
                </div>
      ...
      <section className="py-16 md:py-20 px-6 bg-slate-50/70 border-y border-slate-100">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: GraduationCap, label: isEn ? 'Students' : 'Étudiants', desc: isEn ? 'Jobs that fit around classes' : 'Jobs compatibles avec les cours' },
              { icon: Zap, label: isEn ? 'Extras' : 'Extras', desc: isEn ? 'Short missions, fast pay' : 'Missions courtes, paiement rapide' },
              { icon: Clock, label: isEn ? 'Part-time' : 'Temps partiel', desc: isEn ? 'Recurring contracts, flexible schedules' : 'Contrats récurrents, plannings flexibles' },
              { icon: Calendar, label: isEn ? 'One-off' : 'Missions ponctuelles', desc: isEn ? 'Events, replacements, activity peaks' : 'Événements, remplacements, pics d\'activité' },
            ].map((uc, i) => (
              <div key={i} className="bg-white rounded-xl p-5 border border-slate-200 text-center">
                <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center mx-auto mb-3">
                  <uc.icon size={16} className="text-blue-600" />
                </div>
                <p className="text-sm font-semibold text-slate-900 mb-1">{uc.label}</p>
